    ...
    async request<T = any>(endpoint: string, options: RequestOptions = {}): Promise<ApiResponse<T>> {
        ...
        try {
            ...
            if (response.status === 401) {
                // Clear token cookie
                document.cookie = 'token=; expires=Thu, 01 Jan 2000 00:00:00 UTC; path=/; SameSite=Strict';
            ...
            if (contentType?.includes('application/json')) {
                ...
                data = { data: text, statusCode: response.status, timestamp: new Date().toISOString() };
            }
